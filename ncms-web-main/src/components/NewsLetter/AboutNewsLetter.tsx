...
interface AccordionItem {
  title: string;
  points: string[]; // renamed to match your JSON
}
...
interface VisionMissionSection {
  title?: string;
  sections?: VisionMissionPoint[];
}


interface AboutNewsLetterData {
  AboutSection?: AboutSection;
  VisionMission?: VisionMissionSection;
  AccordionSections?: AccordionItem[];
}

export default function AboutNewsLetter({ data }: { data: AboutNewsLetterData | null }) {
  if (!data) return null;
  ...
  return (
    <div className="bg-white mb-20">
      ...
      {/* Vision & Mission Accordion */}
        {visionMission && Array.isArray(visionMission.sections) && visionMission.sections.length > 0 && (
        <div className="mb-3">
            <div
            ...
            <div className="px-6 py-4 bg-[#F6F6F6] mt-2 rounded-md space-y-6">
                {visionMission.sections.map((sec, idx) => {
                const hasContent =
