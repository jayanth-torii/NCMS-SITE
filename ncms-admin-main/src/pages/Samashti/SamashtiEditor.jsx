...
/* ================================================================== *
 * Samashti Editor — Content Manager
 * Manages the public Samashti (college magazine) page: banner hero,
 * About tagline + story, and the View Editions library of magazine
 * issues (cover image + PDF per edition).
 * ================================================================== */
...
const TABS = [
  ...
  { id: "about", label: "About", desc: "Tagline & story", icon: FiInfo },
  { id: "editions", label: "View Editions", desc: "Magazine issues", icon: FiBookOpen },
...
const SamashtiEditor = () => {
  const canWrite = useSelector((state) => canWritePage(state, "samashti"));
  ...
  const stats = useMemo(() => {
    const editions = (data?.viewEditionSection?.programs || []).length;
    return [{ value: editions, label: "Editions" }];
  }, [data]);
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      await updateContent("/api/samashti", data);
      setSavedAt(Date.now());
    } catch (err) {
  ...
  const editions = d.viewEditionSection || {};
  const programs = editions.programs || [];
  ...
  return (
    <EditorPage>
      ...
      <EditorHeader
        ...
        title="Samashti Editor"
        subtitle="Manage every section of the public Samashti page."
        stats={stats}
        ...
        <Col lg={9}>
          ...
          {tab === "banner" && (
            <Panel style={{ padding: "1.5rem" }}>
              <SectionHead icon={FiImage} title="Banner" subtitle="Top hero banner shown at the top of the Samashti page." />
          ...
          {tab === "about" && (
            <Panel style={{ padding: "1.5rem" }}>
              <SectionHead icon={FiInfo} title="About" subtitle="Tagline and story shown in the About section." />
              <PathField data={data} setData={setData} path={["aboutSection", "year"]} label="Tagline" />
              <PathField
                data={data}
                setData={setData}
                path={["aboutSection", "descriptions"]}
          ...
          {tab === "editions" && (
            <Panel style={{ padding: "1.5rem" }}>
              <SectionHead
                icon={FiBookOpen}
                title="View Editions"
                subtitle="Section heading, intro copy and the magazine issue cards (cover + PDF)."
                right={
                  <Button
                    type="button"
                    onClick={() => addItem(["viewEditionSection", "programs"], { title: "", date: "", image: "", pdf: "" })}
                  ...
                  >
                    <FiPlus size={15} /> Add Edition
                  </Button>
                }
              />
              <PathField data={data} setData={setData} path={["viewEditionSection", "title"]} label="Title" />
              <PathField data={data} setData={setData} path={["viewEditionSection", "description"]} label="Description" type="textarea" rows={3} />
              <PathField
                data={data}
                setData={setData}
                path={["viewEditionSection", "buttons"]}
                label="Buttons (one per line)"
                type="textarea"
                rows={2}
                hint="e.g. Previous, Next"
              />
              <div style={{ marginTop: "1.2rem" }}>
                {programs.length === 0 && (
                  <EmptyState icon={FiBookOpen} title="No editions yet" hint="Add your first magazine edition." />
                )}
                {programs.map((ed, i) => (
                  <ItemCard
                    key={ed.id ?? i}
                    title={`Edition ${i + 1}`}
                    canUp={i > 0}
                    canDown={i < programs.length - 1}
                    onMoveUp={() => moveItem(["viewEditionSection", "programs"], i, -1)}
                    onMoveDown={() => moveItem(["viewEditionSection", "programs"], i, 1)}
                    onRemove={() => removeItem(["viewEditionSection", "programs"], i)}
                  >
                    <Row>
                      <Col md={7}>
                        <PathField data={data} setData={setData} path={["viewEditionSection", "programs", i, "title"]} label="Title" />
                      </Col>
                      <Col md={5}>
                        <PathField data={data} setData={setData} path={["viewEditionSection", "programs", i, "date"]} label="Date / Period" />
                      </Col>
                    </Row>
                    <Row>
                      <Col md={6}>
                        <ImageControl
                          label="Cover image"
                          value={ed.image || ""}
                          onChange={(url) => updateItem(["viewEditionSection", "programs"], i, { ...ed, image: url })}
                        />
                      </Col>
                      <Col md={6}>
                        <FileControl
                          label="Edition PDF"
                          value={ed.pdf || ""}
                          onChange={(url) => updateItem(["viewEditionSection", "programs"], i, { ...ed, pdf: url })}
                        />
                      </Col>
                    </Row>
                  </ItemCard>
                ))}
                <AddBtn onClick={() => addItem(["viewEditionSection", "programs"], { title: "", date: "", image: "", pdf: "" })}>
                  Add edition
                </AddBtn>
              </div>
            </Panel>
          ...
          {tab === "raw" && (
            <Panel style={{ padding: "1.75rem" }}>
              <SectionHead icon={FiCode} title="Advanced (Raw JSON)" subtitle="Full Samashti page content as JSON." />
      ...
      <SaveBar
        ...
        summary={canWrite ? `${stats[0].value} editions` : "You have read-only access to this page."}
      />
    </EditorPage>
  );
};

export default SamashtiEditor;
